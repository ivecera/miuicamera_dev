...
public class EffectNetWorkerWrapper {
    ...
    public <T extends BaseNetResponse> BaseNetResponse execute(EffectRequest effectRequest, IJsonConverter iJsonConverter, Class<T> cls) throws Exception {
        ...
        T convertJsonToObj = iJsonConverter.convertJsonToObj(execute, cls);
        if (convertJsonToObj != null) {
            int status_code = convertJsonToObj.getStatus_code();
            if (status_code == 0) {
                return convertJsonToObj;
            }
            this.mLinkSelector.onApiError(effectRequest.getUrl());
            throw new StatusCodeException(status_code, convertJsonToObj.getMessage());
        }
        this.mLinkSelector.onApiError(effectRequest.getUrl());
