            ...
            if (narrowWidePattern >= 0) {
                ...
                if (patternToChar == '*') {
                    ...
                    if (nextSet2 == size || (i6 << 1) >= i4) {
                        ...
                        if (sb.length() != 0) {
                            float f2 = (float) i;
                            return new Result(this.extendedMode ? decodeExtended(sb) : sb.toString(), null, new ResultPoint[]{new ResultPoint(((float) (findAsteriskPattern[1] + findAsteriskPattern[0])) / 2.0f, f2), new ResultPoint(((float) nextSet) + (((float) i4) / 2.0f), f2)}, BarcodeFormat.CODE_39);
                        }
                        throw NotFoundException.getNotFoundInstance();
                    }
                    throw NotFoundException.getNotFoundInstance();
