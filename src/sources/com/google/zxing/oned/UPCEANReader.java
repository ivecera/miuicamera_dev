...
public abstract class UPCEANReader extends OneDReader {
    ...
    public Result decodeRow(int i, BitArray bitArray, int[] iArr, Map<DecodeHintType, ?> map) throws NotFoundException, ChecksumException, FormatException {
        ...
        if (sb2.length() < 8) {
            ...
            float f2 = (float) i;
            Result result = new Result(sb2, null, new ResultPoint[]{new ResultPoint(((float) (iArr[1] + iArr[0])) / 2.0f, f2), new ResultPoint(((float) (decodeEnd[1] + decodeEnd[0])) / 2.0f, f2)}, barcodeFormat);
            try {
