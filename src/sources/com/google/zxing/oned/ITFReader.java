...
public final class ITFReader extends OneDReader {
    ...
    private static int[] findGuardPattern(BitArray bitArray, int i, int[] iArr) throws NotFoundException {
        ...
        while (i < size) {
            if (bitArray.get(i) != z) {
                ...
                if (i3 != length - 1) {
                    i3++;
                } else if (OneDReader.patternMatchVariance(iArr2, iArr, 0.5f) < MAX_AVG_VARIANCE) {
                    return new int[]{i2, i};
                } else {
                    i2 += iArr2[0] + iArr2[1];
                    int i4 = i3 - 1;
    ...
    public Result decodeRow(int i, BitArray bitArray, Map<DecodeHintType, ?> map) throws FormatException, NotFoundException {
        ...
        if (z) {
            float f2 = (float) i;
            return new Result(sb2, null, new ResultPoint[]{new ResultPoint((float) decodeStart[1], f2), new ResultPoint((float) decodeEnd[0], f2)}, BarcodeFormat.ITF);
        }
