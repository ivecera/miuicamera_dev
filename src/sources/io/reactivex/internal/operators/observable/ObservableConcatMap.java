...
public final class ObservableConcatMap<T, U> extends AbstractObservableWithUpstream<T, U> {
    ...
    static final class ConcatMapDelayErrorObserver<T, R> extends AtomicInteger implements Observer<T>, Disposable {
            ...
            public void onError(Throwable th) {
                ...
                if (concatMapDelayErrorObserver.error.addThrowable(th)) {
                    if (!concatMapDelayErrorObserver.tillTheEnd) {
                        concatMapDelayErrorObserver.f725d.dispose();
                    }
                    concatMapDelayErrorObserver.active = false;
                    concatMapDelayErrorObserver.drain();
                    return;
        ...
        @Override // io.reactivex.Observer
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.f725d, disposable)) {
                this.f725d = disposable;
    ...
    public void subscribeActual(Observer<? super U> observer) {
        if (!ObservableScalarXMap.tryScalarXMapSubscribe(((AbstractObservableWithUpstream) this).source, observer, this.mapper)) {
            ErrorMode errorMode = this.delayErrors;
            if (errorMode == ErrorMode.IMMEDIATE) {
                ((AbstractObservableWithUpstream) this).source.subscribe(new SourceObserver(new SerializedObserver(observer), this.mapper, this.bufferSize));
            } else {
                ((AbstractObservableWithUpstream) this).source.subscribe(new ConcatMapDelayErrorObserver(observer, this.mapper, this.bufferSize, errorMode == ErrorMode.END));
            }
        }
    }
}
