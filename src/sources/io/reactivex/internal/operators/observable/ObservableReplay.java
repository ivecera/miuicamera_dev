...
public final class ObservableReplay<T> extends ConnectableObservable<T> implements HasUpstreamObservableSource<T>, Disposable {
    ...
    static final class SizeAndTimeBoundReplayBuffer<T> extends BoundedReplayBuffer<T> {
        ...
        public void truncate() {
            ...
            while (true) {
                node = node2;
                if (node != null) {
                    int i2 = ((BoundedReplayBuffer) this).size;
                    if (i2 <= this.limit) {
                        if (((Timed) node.value).time() > now) {
                            break;
                        }
                        i++;
                        ((BoundedReplayBuffer) this).size--;
                        node2 = (Node) node.get();
                    } else {
                        i++;
                        ((BoundedReplayBuffer) this).size = i2 - 1;
                        node2 = (Node) node.get();
                    }
                } else {
                    break;
        ...
        public void truncateFinal() {
            ...
            while (true) {
                node = node2;
                if (node != null && ((BoundedReplayBuffer) this).size > 1 && ((Timed) node.value).time() <= now) {
                    i++;
                    ((BoundedReplayBuffer) this).size--;
                    node2 = (Node) node.get();
                } else if (i == 0) {
    ...
    static final class SizeBoundReplayBuffer<T> extends BoundedReplayBuffer<T> {
        ...
        @Override // io.reactivex.internal.operators.observable.ObservableReplay.BoundedReplayBuffer
        public void truncate() {
            if (((BoundedReplayBuffer) this).size > this.limit) {
                removeFirst();
            }
        }
    }
