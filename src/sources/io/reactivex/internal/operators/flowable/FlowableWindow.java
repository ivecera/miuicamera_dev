...
public final class FlowableWindow<T> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    public void subscribeActual(Subscriber<? super Flowable<T>> subscriber) {
        ...
        long j2 = this.size;
        if (j == j2) {
            ((AbstractFlowableWithUpstream) this).source.subscribe((FlowableSubscriber) new WindowExactSubscriber(subscriber, j2, this.bufferSize));
        } else if (j > j2) {
            ((AbstractFlowableWithUpstream) this).source.subscribe((FlowableSubscriber) new WindowSkipSubscriber(subscriber, j2, j, this.bufferSize));
        } else {
            ((AbstractFlowableWithUpstream) this).source.subscribe((FlowableSubscriber) new WindowOverlapSubscriber(subscriber, j2, j, this.bufferSize));
        }
    }
}
